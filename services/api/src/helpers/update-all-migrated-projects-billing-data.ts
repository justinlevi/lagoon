...
//   graphql(ADD_OR_UPDATE_ENVIRONMENT_STORAGE_MUTATION, { input });
// const updateEnvironment = input =>
//   graphql(UPDATE_ENVIRONMENT_MUTATION, { input });
// const updateProjectBillingGroup = input =>
//   graphql(UPDATE_PROJECT_BILLING_GROUP, { input });
// const updateProjectMetadata = input =>
//   graphql(UPDATE_PROJECT_METADATA, { input });
...
const main = async () => {
  ...
  oldProjects.forEach(async project => {
    ...
    console.log(
      `${project.name} PROJECT CREATED DATE: ${newProjectCreatedDate}`
    );
    ...
    environmentsOld.forEach(async environmentOld => {
      ...
      environmentOld.storages.forEach(async storage => {
        ...
        try {
          ...
          const addOrUpdateEnvironmentStorageResult = await addOrUpdateEnvironmentStorage(
            addOrUpdateEnvironmentStorageInput
          );
        } catch (error) {
          console.debug(error);
        }
        ...
        try {
          ...
          const updateEnvironmentCreatedDateResult = await updateEnvironment(
